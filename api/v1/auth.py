...
async def agent_login(credentials: AgentLogin, db: Session = Depends(get_db)):
    # On cherche l'agent par email (username)
    ...
    if not agent.mot_de_passe_hash or not verify_password(
        credentials.password, agent.mot_de_passe_hash
    ):
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="Email ou mot de passe incorrect",
    ...
    # IMPORTANT: token_type doit être renvoyé sinon TokenResponse peut planter (500)
...
async def producteur_login(credentials: ProducteurLogin, db: Session = Depends(get_db)):
    # On cherche le producteur par téléphone
    producteur = (
        db.query(Producteur).filter(Producteur.telephone == credentials.telephone).first()
    )
